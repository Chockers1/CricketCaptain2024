...
def display_bat_view():
    ...
    if 'bat_df' in st.session_state:
        ...
        if bat_team_choice and 'All' not in bat_team_choice:
            filtered_df = filtered_df[filtered_df['Bat_Team_y'].isin(bat_team_choice)]  # Changed from Bat_Team
        if bowl_team_choice and 'All' not in bowl_team_choice:
            filtered_df = filtered_df[filtered_df['Bowl_Team_y'].isin(bowl_team_choice)]  # Changed from Bowl_Team
        ...
        # Apply year and position filters
...
###---------------------------------------------FORMAT STATS-------------------------------------------------------------------###
        ...
        # Display the filtered and aggregated career statistics without the index
 ...
 ###---------------------------------------------SESAON STATS-------------------------------------------------------------------###
        ...
        # Add colors dynamically based on selected names
        for name in name_choice:
            if name not in color_map:
                color_map[name] = random_color()  # Assign random color
        ...
        if 'All' in name_choice:
            ...
            all_players_stats['Avg'] = (all_players_stats['Runs'] / all_players_stats['Out']).round(2).fillna(0)
            all_players_stats['SR'] = ((all_players_stats['Runs'] / all_players_stats['Balls']) * 100).round(2).fillna(0)
        ...
        for name in name_choice:
            if name != 'All':  # Skip 'All' as we've already handled it
                ...
                # Summarize player stats for the graph
                ...
                player_yearly_stats['Avg'] = (player_yearly_stats['Runs'] / player_yearly_stats['Out']).round(2).fillna(0)
                player_yearly_stats['SR'] = ((player_yearly_stats['Runs'] / player_yearly_stats['Balls']) * 100).round(2).fillna(0)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    mode='lines+markers', 
                    name=name,  # Same name for all traces
                    legendgroup=name,  # Group by player name
                    marker=dict(color=player_color, size=8),  # Set unique color
        ...
        # Update layout with matching style
        fig.update_layout(
            showlegend=True,
            title_text=None)
        st.plotly_chart(fig)
        ...
        # Add Runs trace
        fig.add_trace(
            go.Bar(x=season_stats_df['Year'], y=season_stats_df['Runs'], name='Runs', marker_color='#f84e4e')
        )

        st.markdown("<h3 style='color:#f04f53; text-align: center;'>Runs Per Year</h3>", unsafe_allow_html=True)
        ...
        st.error("Batting statistics are not available. Please ensure you have processed the scorecards on the Home page.")

######---------------------------------------LATEST INNINGS STATS TAB-------------------------------


    # Create the latest_inns_df by grouping by 'Name', 'Match_Format', 'Date', and 'Innings' with the required aggregations
    latest_inns_df = filtered_df.groupby(['Name', 'Match_Format', 'Date', 'Innings']).agg({
        'Bat_Team_y': 'first',  # Changed from Bat_Team
        'Bowl_Team_y': 'first',  # Changed from Bowl_Team
        'How Out': 'first',    
        'Balls': 'sum',        
        'Runs': ['sum'],       
        '4s': 'sum',           
...
###---------------------------------------------OPPONENTS STATS-------------------------------------------------------------------###
   # Calculate opponents statistics
    opponents_stats_df = filtered_df.groupby(['Name', 'Bowl_Team_y']).agg({  # Changed from Bowl_Team
        'File Name': 'nunique',
        'Batted': 'sum',
        'Out': 'sum',
        'Not Out': 'sum',    
    ...
    # Calculate average runs against opponents
    opponent_stats_df = filtered_df.groupby('Bowl_Team_y').agg({  # Changed from Bowl_Team
        'Runs': 'sum',
        'Out': 'sum',
        'Balls': 'sum'
    }).reset_index()
...
# Convert the 'Date' column to datetime format for proper chronological sorting
    ...
    # Create the cumulative_stats_df by grouping by 'Name', 'Match_Format', and 'Date' with the required aggregations
    ...
    # Add Innings as an aggregator
    cumulative_stats_df['Inns'] = cumulative_stats_df['Batted'].where(cumulative_stats_df['Batted'] == 1, 0).cumsum()

    # Create the cumulative_inns column
    cumulative_stats_df = cumulative_stats_df.sort_values(by=['Name', 'Match_Format', 'Date', 'Inns'])  # Sort the DataFrame
    cumulative_stats_df['Cumulative Matches'] = cumulative_stats_df.groupby(['Name', 'Match_Format'])['Batted'].cumsum()  # Cumulative sum of Matches
    cumulative_stats_df['Cumulative Runs'] = cumulative_stats_df.groupby(['Name', 'Match_Format'])['Runs'].cumsum()  # Cumulative sum of Runs
    cumulative_stats_df['Cumulative 100s'] = cumulative_stats_df.groupby(['Name', 'Match_Format'])['100s'].cumsum()  # Cumulative sum of 100s
    cumulative_stats_df['Cumulative Balls'] = cumulative_stats_df.groupby(['Name', 'Match_Format'])['Balls'].cumsum()  # Cumulative sum of Balls
    cumulative_stats_df['Cumulative Outs'] = cumulative_stats_df.groupby(['Name', 'Match_Format'])['Out'].cumsum()  # Cumulative sum of Outs

    # Calculate cumulative innings
    cumulative_stats_df['Cumulative Inns'] = cumulative_stats_df.groupby(['Name', 'Match_Format'])['Inns'].cumsum()

    # Calculate cumulative average and strike rate
    cumulative_stats_df['Cumulative Avg'] = cumulative_stats_df['Cumulative Runs'] / cumulative_stats_df['Cumulative Outs'].replace(0, np.nan)  # Avoid division by zero
    cumulative_stats_df['Cumulative SR'] = (cumulative_stats_df['Cumulative Runs'] / cumulative_stats_df['Cumulative Balls'].replace(0, np.nan)) * 100  # Avoid division by zero
    # Calculate runs per match
    cumulative_stats_df['Runs per Match'] = cumulative_stats_df['Cumulative Runs'] / cumulative_stats_df['Cumulative Matches']

    # Drop the specified columns
    columns_to_drop = ['<25&Out', 'Caught', 'Bowled', 'LBW', 'Run Out', 'Stumped', 'Inns']
    cumulative_stats_df = cumulative_stats_df.drop(columns=columns_to_drop)

    # Flatten multi-level columns (if needed after dropping)
    cumulative_stats_df.columns = [
        'Name', 'Match_Format', 'Date', 'Matches', 'Batted', 
        'Bat Team', 'Bowl Team', 'Out', 'Not Out', 'Balls',  # Changed column names to be more readable
        'Runs', '4s', '6s', '50s', '100s', '200s', 
        'Cumulative Matches', 'Cumulative Runs', 
        'Cumulative 100s', 'Cumulative Balls', 'Cumulative Outs', 
        'Cumulative Inns', 'Cumulative Avg', 'Cumulative SR', 'Runs per Match'
    ]
    cumulative_stats_df = cumulative_stats_df.sort_values(by='Cumulative Matches', ascending=False)

    # Store the final DataFrame in df_bat
    df_bat = cumulative_stats_df.copy()  

    # Display the batting statistics in the Streamlit app
    ...
    def create_plot(data, x_col, y_col, title, color_map, show_legend=True):
        ...
        fig.update_layout(
            ...
            legend=dict(
                ...
                xanchor="center",   # anchor in center
...
###--------------------------------------BLOCK STATS------------------------------------------#######
    ...
    # Add match block column based on Cumulative Matches (now called Match Range)
