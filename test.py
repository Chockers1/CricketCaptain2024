...
def process_data():
    ...
    # Sort DataFrame by Month_ID (descending) and then by Rank (ascending)
    df.sort_values(by=['Month_ID', 'Rank'], ascending=[False, True], inplace=True)
...
def server(input, output, session):
    ...
    def rankings_table():
        df = process_data()
        return df.head(20)  # Display the top 20 rows
